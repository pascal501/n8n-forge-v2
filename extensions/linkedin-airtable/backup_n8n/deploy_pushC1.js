...
const body = fs.readFileSync(dir + "\\push_C1.json", "utf8");
...
(async () => {
  ...
  console.log("✅ PUSH C1 créé. id =", id);
  const g = await (await fetch("https://usine.dinaou.com/api/v1/workflows/" + id, { headers: H })).json();
  const wf = g.data || g;
  const upd = wf.nodes.find(n => n.name === "Contacts : poser lien Entreprise");
  console.log("nœuds :", wf.nodes.map(n => n.name).join(" → "));
  console.log("update op :", upd.parameters.operation, "| match :", JSON.stringify(upd.parameters.columns.matchingColumns));
})();
