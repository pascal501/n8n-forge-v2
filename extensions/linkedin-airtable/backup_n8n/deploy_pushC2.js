...
const body = fs.readFileSync(dir + "\\push_C2.json", "utf8");
...
(async () => {
  ...
  console.log("✅ PUSH C2 créé. id =", id);
  const g = await (await fetch("https://usine.dinaou.com/api/v1/workflows/" + id, { headers: H })).json();
  const wf = g.data || g;
  const cr = wf.nodes.find(n => n.name === "Cibles : Créer (incuber)");
  console.log("create → base LCM :", cr.parameters.base.value === "appiuwspImLMu7KJQ", "| op :", cr.parameters.operation);
  console.log("executeOnce (Companies+Cibles) :", wf.nodes.filter(n => n.executeOnce).length, "/ 2");
})();
