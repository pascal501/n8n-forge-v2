...
const body = fs.readFileSync(dir + "\\urlfinder.json", "utf8");
...
(async () => {
  ...
  if (!r.ok) { console.log("ERREUR:", r.status, JSON.stringify(j).slice(0, 500)); return; }
  const id = (j.data || j).id;
  console.log("✅ Trouveur URL LinkedIn créé. id =", id);
